import { getGamesByGraphicCard, getCommentsFromSpecificGame, getGames, getSpecificGame, postGames } from "../src/endpoints";
...
global.fetch = jest.fn();
  ...
  describe('Testando a função postGames', () => {
    ...
    it('Deve falhar ao criar um novo registro de jogo', async () => {
      ...
      fetch.mockResolvedValue({
        ok: true,
        json: () => Promise.resolve(new Error("Falha")),
      });

      const result = await postGames(newGame);
      
      expect(result).toBe("Erro ao adicionar jogo: Falha");
    });
  })
